...
global solutionTable solutions len ind ySolutions table bol table1;
solutionTable = [];
solutions = [];
table = [];
table1 = [];
...
elseif selected_item == 5 || selected_item == 6
...
global solutionTable solutions len ind ySolutions table step table1;
solutionTable = [];
solutions = [];
table = [];
table1 = [];
    ...
    try 
        ...
        switch selected_item    
            case 1
                ...
                [table1,solutions,err5] = JacobiIterative(A,B,initial,iteration,tolerance);
            ...
            case 6
                [table,solutions,err] = JacobiIterative(A,B,initial,iteration,tolerance);
                if(err == 0)
                    len = size(A);
                    colNames = getColNames(length(initial));
                    set(handles.uitable2,'ColumnName',colNames);
                    ...
                    msgbox('Jacob Siedel : cannot get solution...','Error','error');
                    return
                end
            otherwise
        end 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global solutionTable solutions len ind ySolutions step bol;
if bol == 0
    return;
end
[r,c] = size(solutionTable);
    if (ind > r)
        ...
        if(get(handles.method_menu,'Value') == 3)
            if(ind == r+1)
                set(handles.uitable1,'Columnname',{'Y Solutions'});
                set(handles.uitable1,'Data',ySolutions);
                ind = r+2;
                step = step + 1;
            else
                if ind ~= r+3
                set(handles.uitable1,'Columnname',{'X Solutions'});
        ...
        % show solutions [x]
        if ind ~= r+2
        set(handles.uitable1,'Columnname',{'Solutions'});
        set(handles.uitable1,'Data',solutions);
        ind = r + 2;
        step = step + 1;
        return;
    ...
    set(handles.uitable1,'Columnname',{sprintf('Original'),sprintf('Matrix')});
    step = step +1;
    ...
    set(handles.uitable1,'Data',cell);
         ...
         if get(handles.method_menu,'Value') == 3
             if ind == r-len+1
                  set(handles.uitable1,'Columnname',{sprintf('Matrix L')});
                  step = step +1;
                  cell = getCell(ind,len,solutionTable);
                  ind = ind + len;
                  set(handles.uitable1,'Data',cell);
             else
                  set(handles.uitable1,'Columnname',{sprintf('Step %d' ,step),sprintf('Matrix U')});
                  step = step +1;
                  cell = getCell(ind,len,solutionTable);
                  ind = ind + len;
                  set(handles.uitable1,'Data',cell);
            ...
            set(handles.uitable1,'Columnname',{sprintf('Step %d' ,step)});
            step = step +1;
            cell = getCell(ind,len,solutionTable);
            ind = ind + len;
            set(handles.uitable1,'Data',cell); 
...
if isequal(file,0)
   disp('User selected Cancel');
        ...
        elseif strcmp(TypeOfMethod,'JacobiIterative')
            set(handles.method_menu,'Value',6);
        end
...
[r,c] = size(solutionTable);
    ...
    if(ind == r + 2)
        if get(handles.method_menu,'Value') == 3
            ...
            step = step -1;
            set(handles.uitable1,'Columnname',{sprintf('Step %d' ,step-1)});
            ind = r + 1 ;
            cell = getCell(r-len+1,len,solutionTable);
            set(handles.uitable1,'Data',cell);
        end
    ...
    ind = ind - len;
        ...
        if get(handles.method_menu,'Value') == 3
            ...
            set(handles.uitable1,'Columnname',{sprintf('Step %d' ,step-2*1),sprintf('Matrix U')});
            step = step - 1;
            cell = getCell(ind-2*len,len,solutionTable);
            set(handles.uitable1,'Data',cell);
            ind = ind - len;
           ...
           set(handles.uitable1,'Columnname',{sprintf('Step %d' ,step-2*1)});
           step = step - 1;
           cell = getCell(ind-2*len,len,solutionTable);
           set(handles.uitable1,'Data',cell);
           ind = ind - len;
        ...
        while (rowCounter<=length(1)&&flag==0)
            ...
            while(columnCounter<=length(2))
                help(rowCounter,columnCounter)=help(diagonalCounter,columnCounter)*factor+help(rowCounter,columnCounter);
                  if(isnan(help(rowCounter,columnCounter)) || isinf(help(rowCounter,columnCounter)))
                    flag = 1;
                  end 
                columnCounter=columnCounter+1;
            end
...
if (condition==0)
    ...
    while(diagonalCounter<=lengthh(1))
        ...
        while (rowCounter>0)
            ...
            while (columnCounter<=lengthh(2))
                ...
                if(isnan(finalMatrix( rowCounter,columnCounter)) || isinf(finalMatrix( rowCounter,columnCounter)))
                    condition = 1;
                end 
                 columnCounter=columnCounter+1;
            end
    ...
    while ( diagonalCounter<=lengthh(1))
       ...
       if(isnan(variables(diagonalCounter)) || isinf(variables(diagonalCounter)))
             condition = 1;
             return;
       end 
       diagonalCounter=diagonalCounter+1; 
...
if get(handles.method_menu,'Value') == 6 || get(handles.method_menu,'Value') == 5
    if (len(1) == len(2))
        Hf_1 = figure(1); 
        set(Hf_1,'NumberTitle','off','Name','(Roots) vs (Number of iterations)');
        %set(Hf_1,'Position',[425 150 300 300]);
...
elseif get(handles.method_menu,'Value') == 1
    if (len(1) == len(2))
        Hf_1 = figure(1); 
        set(Hf_1,'NumberTitle','off','Name','(Roots) vs (Number of iterations)');
        movegui(Hf_1,[120 150]);
         
        Legend1 = cell(len(1),1);
        for i = 1:len(1)
              plot([0;table(:,1)],[table(1,i+1);table(:,i+len(1)+1)]);
              Legend1{i} = strcat('X',num2str(i));
              hold on;
        end
        leg1 = legend(Legend1);
        title(leg1,'Gauss Siedel');
        xlabel('Number of Iterations', 'FontSize', 12,'color','b');
        ylabel('Roots', 'FontSize', 12,'color','b');
        title('The behaviour Of Roots According to Number of Iterations', 'FontSize', 12,'color','b');
        grid on;
        ...
        Hf_2 = figure(2); 
        set(Hf_2,'NumberTitle','off','Name','(Roots) vs (Number of iterations)');
        movegui(Hf_2,[700 150]);
        
        Legend2 = cell(len(1),1);
        for i = 1:len(1)
              plot([0;table1(:,1)],[table1(1,i+1);table1(:,i+len(1)+1)]);
              Legend2{i} = strcat('X',num2str(i));
              hold on;
        end
        leg2 = legend(Legend2);
        title(leg2,'Jacob Siedel');
        xlabel('Number of Iterations', 'FontSize', 12,'color','b');
        ylabel('Roots', 'FontSize', 12,'color','b');
        title('The behaviour Of Roots According to Number of Iterations', 'FontSize', 12,'color','b');
        grid on;
...
function[table,sol,flagError] = JacobiIterative(A, b, x0, iterations, error)
time = tic;
flagError = 0;
try
    num_of_variables = size(A,2);
...
%             er(i,j) = abs(x(i,j) - x(i - 1,j));
    ...
    fileName = strcat(pwd,'\outputJacobiIterative.txt');
    fileID = fopen(fileName,'wt');
    [r,c] = size(table);
    colNames = getColNames(double((c-2)/3));
